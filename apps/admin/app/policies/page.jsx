      ...
      sections: [
        {
          ...
          content: "Items must be returned within 30 days of purchase in their original condition.",
          isOrdered: false,
          listItems: [
  ...
  const addListItem = (policyType, sectionId) => {
    setPolicies((prev) => ({
      ...
      [policyType]: {
        ...prev[policyType],
        sections: prev[policyType].sections.map((section) =>
          section.id === sectionId ? { ...section, listItems: [...section.listItems, "New list item"] } : section,
        ),
      },
    }))
  }
      ...
      [policyType]: {
        ...
        sections: prev[policyType].sections.map((section) =>
          section.id === sectionId
            ? {
                ...section,
                listItems: section.listItems.map((item, index) => (index === itemIndex ? value : item)),
              }
            : section,
        ),
      },
      ...
      [policyType]: {
        ...
        sections: prev[policyType].sections.map((section) =>
          section.id === sectionId
            ? {
                ...section,
                listItems: section.listItems.filter((_, index) => index !== itemIndex),
              }
            : section,
        ),
      },
        ...
        <div className="p-6">
          <div className="max-w-7xl mx-auto">
            <div className="bg-white rounded-xl shadow-lg p-8">
              ...
              <p className="text-gray-600">Manage your store policies and legal documents</p>
            </div>
          </div>
        </div>
        <div className="p-6">
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <p className="text-gray-600">Manage your store's legal policies and terms</p>
                </div>
                <button
                  onClick={handleSave}
                    ...
                    {Object.entries(policies).map(([key, policy]) => (
                      ...
                      >
                        ...
                        <div className="flex-1">
                          ...
                          <div className="text-xs text-gray-500">Updated: {policy.lastUpdated}</div>
                        </div>
                      </button>
                    ))}
                  </nav>
                    ...
                    <div className="flex justify-between items-center">
                      <input
                        ...
                        onChange={(e) =>
                          setPolicies((prev) => ({
                            ...prev,
                            [activePolicy]: { ...prev[activePolicy], title: e.target.value },
                          }))
                        }
                        className="text-2xl font-bold text-gray-900 bg-transparent border-none focus:outline-none focus:ring-0 p-0 flex-1"
                      />
                      <p className="text-sm text-gray-500 mt-1">Last updated: {currentPolicy.lastUpdated}</p>
                    </div>
                  </div>
                        ...
                        <div key={section.id} className="border border-gray-200 rounded-lg p-4">
                          <div className="flex justify-between items-start mb-4">
                            <input
                              ...
                              onChange={(e) => updateSection(activePolicy, section.id, "heading", e.target.value)}
                            ...
                            <button
                              onClick={() => {
                                setPolicies((prev) => ({
                                  ...
                                  [activePolicy]: {
                                    ...prev[activePolicy],
                                    sections: prev[activePolicy].sections.filter((s) => s.id !== section.id),
                                  },
                                }))
                              }}
                          ...
                          <textarea
                            ...
                            onChange={(e) => updateSection(activePolicy, section.id, "content", e.target.value)}
                            rows={3}
                            ...
                            <div className="flex justify-between items-center">
                              <label className="flex items-center space-x-2">
                                <input
                                  ...
                                  onChange={(e) =>
                                    updateSection(activePolicy, section.id, "isOrdered", e.target.checked)
                                  }
                                  className="rounded"
                                />
                              ...
                              <button
                                onClick={() => addListItem(activePolicy, section.id)}
                            ...
                            {section.listItems.map((item, itemIndex) => (
                              <div key={itemIndex} className="flex items-center space-x-2">
                                <span className="text-gray-500 text-sm w-6">
                                  {section.isOrdered ? `${itemIndex + 1}.` : "•"}
                                </span>
                                <input
                                  type="text"
                                  value={item}
                                  onChange={(e) => updateListItem(activePolicy, section.id, itemIndex, e.target.value)}
                                  className="flex-1 px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                />
                                <button
                                  onClick={() => removeListItem(activePolicy, section.id, itemIndex)}
