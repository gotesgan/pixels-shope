...
import { Plus, Search, Edit, Trash2, ChevronRight, ChevronDown, Folder, FolderOpen, X } from "lucide-react"
...
export default function CategoriesPage() {
  ...
  const fetchCategories = async () => {
    setLoading(true)
    try {
      const res = await fetch("http://localhost:3001/api/v1/products/categorys", {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch("http://localhost:3001/api/v1/products/category", {
        method: "POST",
  ...
  const handleDelete = async (categoryId) => {
    ...
    try {
      const res = await fetch(`http://localhost:3001/api/v1/products/category/${categoryId}`, {
        method: "DELETE",
      ...
      <div className="border-b border-gray-100 last:border-b-0">
        ...
        >
          <div className="flex items-center space-x-3 flex-1">
            {hasChildren ? (
              <button onClick={() => toggleExpanded(category._id)} className="p-1 hover:bg-gray-200 rounded">
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
              ...
              <div className="bg-green-50 p-4 rounded-lg">
                <div className="text-2xl font-bold text-green-700">{organizedCategories.length}</div>
                <div className="text-sm text-green-600">Root Categories</div>
              </div>
              <div className="bg-purple-50 p-4 rounded-lg">
                <div className="text-2xl font-bold text-purple-700">{categories.filter((c) => c.parent).length}</div>
