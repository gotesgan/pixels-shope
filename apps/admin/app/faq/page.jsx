...
export default function FAQPage() {
  ...
  const [faqs, setFaqs] = useState([
    ...
    },
    {
      ...
      answer: "We accept all major credit cards, debit cards, PhonePe, and cash on delivery for eligible orders.",
      category: "Payment",
...
const handleSave = async (faq) => {
  ...
  const updateFaq = (id, field, value) => {
    setFaqs(
        faqs.map((faq) =>
            faq.id === id ? { ...faq, [field]: value, updatedAt: new Date().toISOString().split("T")[0] } : faq
        )
    )
  }
              ...
              <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                  <div className="bg-blue-50 p-4 rounded-lg">
                    <div className="text-2xl font-bold text-blue-700">{faqs.length}</div>
                  ...
                  <div className="bg-orange-50 p-4 rounded-lg">
                    <div className="text-2xl font-bold text-orange-700">
                      {faqs.filter((f) => f.updatedAt === new Date().toISOString().split("T")[0]).length}
                    </div>
                        ...
                        <div className="flex justify-between items-start">
                          <div className="flex-1">
                            <div className="flex items-center space-x-3 mb-2">
                              <span className="bg-blue-100 text-blue-800 text-xs px-2 py-1 rounded">{faq.category}</span>
                              <span className="text-xs text-gray-500">Updated: {faq.updatedAt}</span>
                            </div>
                            <input
                                type="text"
                                value={faq.question}
  ...
  <span>Save Changes</span>
                ...
                {filteredFaqs.length === 0 && (
                    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-12 text-center">
                      ...
                      <p className="text-gray-600 mb-4">Create your first FAQ to help customers find answers quickly</p>
                      <button
                          onClick={addNewFaq}
