...
export default function ContactPage() {
  ...
  const [contactData, setContactData] = useState({
    information: [
      ...
      { type: "EMAIL", value: "contact@mystore.com", label: "Business Email", isPrimary: true },
      { type: "ADDRESS", value: "123 Business Street, City, State 12345", label: "Main Office", isPrimary: true },
      { type: "WHATSAPP", value: "+91 9876543210", label: "WhatsApp Support", isPrimary: false },
    ],
    businessHours: [
    ...
    socialLinks: [
      { platform: "FACEBOOK", url: "https://facebook.com/mystore", isActive: true },
      { platform: "INSTAGRAM", url: "https://instagram.com/mystore", isActive: true },
      { platform: "TWITTER", url: "https://twitter.com/mystore", isActive: true },
    ],
    locations: [
      {
        name: "Main Office",
            ...
            <div className="space-y-6 mt-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
                  <div className="text-2xl font-bold text-blue-700">{contactData.submissions.length}</div>
                  <div className="text-sm text-blue-600">Total Inquiries</div>
                </div>
                <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
                  <div className="text-2xl font-bold text-yellow-700">
                    {contactData.submissions.filter((s) => s.status === "PENDING").length}
                  </div>
                  <div className="text-sm text-yellow-600">Pending</div>
                </div>
                <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
                  <div className="text-2xl font-bold text-green-700">
                    {contactData.submissions.filter((s) => s.status === "RESPONDED").length}
                  </div>
                  <div className="text-sm text-green-600">Responded</div>
                </div>
                <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
                  <div className="text-2xl font-bold text-purple-700">{contactData.locations.length}</div>
                            ...
                            <div className="space-y-2">
                              <select
                                ...
                                onChange={(e) => {
                                  ...
                                  newInfo[index] = { ...newInfo[index], type: e.target.value }
                                  setContactData((prev) => ({ ...prev, information: newInfo }))
                                }}
                          ...
                          >
                            ...
                            <input
                              ...
                              onChange={(e) => {
                                ...
                                setContactData((prev) => ({ ...prev, businessHours: newHours }))
                              }}
                              placeholder="Days"
                            ...
                            <label className="flex items-center space-x-2">
                              <input
                                ...
                                onChange={(e) => {
                                  const newHours = [...contactData.businessHours]
                                  newHours[index] = { ...newHours[index], isActive: e.target.checked }
                                  setContactData((prev) => ({ ...prev, businessHours: newHours }))
                                }}
                                className="rounded"
                              />
                          ...
                          <div key={index} className="border border-gray-300 rounded-lg p-4">
                            ...
                            <div className="space-y-2">
                              <select
                                ...
                                onChange={(e) => {
                                  const newSocial = [...contactData.socialLinks]
                                  newSocial[index] = { ...newSocial[index], platform: e.target.value }
                                  setContactData((prev) => ({ ...prev, socialLinks: newSocial }))
                                }}
                                className="w-full px-3 py-2 border border-gray-300 rounded-lg"
                              >
                          ...
                          <div key={index} className="border border-gray-300 rounded-lg p-4">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                              <input
                                ...
                                value={location.name}
                                onChange={(e) => {
                                  const newLocations = [...contactData.locations]
                                  newLocations[index] = { ...newLocations[index], name: e.target.value }
                                  setContactData((prev) => ({ ...prev, locations: newLocations }))
                                }}
                              ...
                              <input
                                ...
                                onChange={(e) => {
                                  const newLocations = [...contactData.locations]
                                  newLocations[index] = { ...newLocations[index], postalCode: e.target.value }
                                  setContactData((prev) => ({ ...prev, locations: newLocations }))
                                }}
                              ...
                              <input
                                ...
                                onChange={(e) => {
                                  const newLocations = [...contactData.locations]
                                  newLocations[index] = { ...newLocations[index], country: e.target.value }
                                  setContactData((prev) => ({ ...prev, locations: newLocations }))
                                }}
                                placeholder="Country"
                                className="px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                          ...
                          <div key={submission.id} className="border border-gray-300 rounded-lg p-4">
                            ...
                            <div className="flex justify-between items-center text-sm text-gray-500">
                              ...
                              <div className="flex space-x-2">
                                <button className="text-green-600 hover:text-green-700">Mark as Responded</button>
                                <button className="text-red-600 hover:text-red-700">Mark as Spam</button>
                              </div>
                            </div>
                          </div>
                        ))}
