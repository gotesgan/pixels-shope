    ...
    try {
      ...
      if (mediaData.uploadedImages && mediaData.uploadedImages.length > 0) {
        ...
        if (filename) {
          ...
          console.error(`No filename found in mediaHandler result for ${fieldName}`);
        }
      } else {
        console.error(`No uploadedImages array in mediaHandler result for ${fieldName}`);
      }
    } catch (error) {
...
export const createAboutPage = async (req, res) => {
  ...
  try {
    ...
    console.log("Files structure:", typeof files, Array.isArray(files) ? "array" : "object");
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const aboutCompanyResult = formData.aboutCompany ? 
        await createAboutCompany(tx, newAboutPage.id, formData.aboutCompany, uploadedMedia, storeId) : 
        null;

      // 3. Create OurBrand section if data provided
      const ourBrandResult = formData.ourBrand ? 
        await createOurBrand(tx, newAboutPage.id, formData.ourBrand, uploadedMedia, storeId) : 
        null;

      // 4. Create CompanyFact section if data provided
      const companyFactResult = formData.companyFact ? 
        await createCompanyFact(tx, newAboutPage.id, formData.companyFact, uploadedMedia, storeId) : 
        null;

      // 5. Create AboutSections if data provided
      const aboutSectionsResult = formData.aboutSections?.length > 0 ? 
        await createAboutSections(tx, newAboutPage.id, formData.aboutSections, uploadedMedia, storeId) : 
        [];
