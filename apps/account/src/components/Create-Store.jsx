  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        'http://localhost:3000/api/v1/user/create-store',
        {
          method: 'POST',
          headers: {
      ...
      <div className="max-w-md mx-auto p-6 bg-white rounded-lg shadow-md mt-16">
        <div className="text-center">
          ...
          <div className="bg-gray-50 p-4 rounded-lg mb-6 text-left">
            ...
            <p>
              <strong>Store ID:</strong> {storeData?.store?.id}
            </p>
            <p>
              <strong>Domain:</strong> {storeData?.store?.domain}
            </p>
            <p>
              <strong>Name:</strong> {storeData?.store?.storeInfo?.name}
            </p>
            <p>
              <strong>Business Type:</strong>{' '}
              {storeData?.store?.storeInfo?.businessTypes}
            </p>
          </div>
